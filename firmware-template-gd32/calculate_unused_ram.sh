...
used_ram=$(grep ".ram" "$size_file" | head -n 2 | tail -n 1 | awk '{print $2}')

total_ram=$(grep "RAM (xrw)" "$linker_script" | awk '{print $NF}' | sed 's/K$//' | awk '{printf "%d", $0 * 1024}')
unused_ram=$(( $(echo $total_ram)  - $(echo $used_ram)))
...
unused_ramadd=$(( $(echo $total_ramadd) - $(echo $used_data) - $(echo $used_bss) - $(echo $used_ramadd)))
